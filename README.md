...
- Next.js (App Router)
- React
- PostgreSQL ([Neon](https://neon.tech)) via `@neondatabase/serverless` and server-side API routes (`src/app/api/**`)
- Tailwind CSS
...
- Node.js
- A Neon database (or any Postgres instance compatible with `postgresql://` URLs)
...
```bash
# Neon connection string (Dashboard → Connection details → URI).
# For serverless hosting (e.g. Vercel), prefer the pooled connection string / pooler host.
DATABASE_URL="postgresql://USER:PASSWORD@HOST/neondb?sslmode=require"
```

Important:
- `DATABASE_URL` is **server-only**. Do **not** commit real credentials; use your host’s secret store in production.
- `NEON_DATABASE_URL` is accepted as an alias for `DATABASE_URL`.
...
Run the SQL in `neon/schema.sql` in the Neon SQL Editor (or any Postgres client). This creates:
`games`, `players`, `rounds`, `wines`, `round_wines`, `round_submissions`, `gambit_submissions`.
...
drop table if exists public.gambit_submissions cascade;
drop table if exists public.wines cascade;
drop table if exists public.players cascade;
drop table if exists public.games cascade;
```

2) Then run the full contents of `neon/schema.sql`.

Notes:
- `wines.price` is stored as `numeric(10,2)` for exact cents (no float rounding).
- Re-running `neon/schema.sql` is safe; it includes an idempotent step to keep `wines.price` aligned.
