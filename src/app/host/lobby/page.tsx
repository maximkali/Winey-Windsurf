  ...
  return (
    <WineyShell maxWidthClassName="max-w-[860px]">
      <main className="winey-main">
        <div className="winey-stack">
          <WineyCard className="winey-card-pad">
            ...
            <div className="mt-3 rounded-[var(--winey-radius)] border border-[color:var(--winey-border)] bg-white px-4 py-3 text-center shadow-[var(--winey-shadow-sm)]">
              <p className="text-[13px]">
                <span className="text-[color:var(--winey-title)] font-semibold">●</span>{' '}
                <span className="font-semibold">Game Code:</span> {state?.gameCode ?? gameCode ?? ' – '}
              </p>
              <p className="mt-1 text-[12px] text-[color:var(--winey-muted)] tabular-nums">
              ...
              {(state?.players ?? []).map((p) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {uid && p.uid === uid ? <span className="text-[12px] text-[color:var(--winey-muted)]">(Me)</span> : null}
                    {p.uid === uid ? <span className="text-[12px] text-[color:var(--winey-muted)]">(Admin)</span> : null}
                  {p.uid === uid && p.isCompeting === false ? (
                    <span className="text-[12px] text-[color:var(--winey-muted)]">(Not playing)</span>
                  ) : null}
                  </div>
            ...
            <div className="mt-4 text-center">
              <Link href={organizeRoundsHref} className="text-[13px] text-[color:var(--winey-accent-link)] underline">
                Back to Organize Rounds
              </Link>
              <span className="mx-2 text-[13px] text-[color:var(--winey-muted)]">·</span>
            ...
            <div className="mt-3">
              <ScoringDetails tastingConfig={tastingConfig} />
            </div>
          </WineyCard>
        </div>
      </main>
      ...
      <ConfirmModal
        ...
        description="Once started, no new players can join. All current players will advance to Round 1."
