      ...
      >
        <div className="text-center">
          ...
          <div className="mt-2 text-center">
            {gameCode ? (
              <Link href={`/player/join?gameCode=${encodeURIComponent(gameCode)}`} className="text-[13px] text-[color:var(--winey-accent-link)] underline">
                Rejoin
              </Link>
            ) : (
              <button type="button" onClick={() => (onClose ? onClose() : router.back())} className="text-[13px] text-[color:var(--winey-accent-link)] underline">
                Back
              </button>
            )}
          </div>
    ...
    >
      {variant === 'inline' ? (
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-[12px] text-[color:var(--winey-muted)]">Copy a player’s rejoin link or remove them from the game.</p>
        </div>
      )}
      ...
      {!isHost ? (
        <div className="mt-3 rounded-[var(--winey-radius)] border border-[color:var(--winey-border)] bg-[color:var(--winey-surface)] px-4 py-3 text-center shadow-[var(--winey-shadow-sm)]">
          ...
          <p className="mt-1 text-[12px] text-[color:var(--winey-muted)]">Open this from the host account.</p>
        </div>
      ) : null}
      ...
      {isHost ? (
        <div className="mt-3 rounded-[var(--winey-radius)] border border-[color:var(--winey-border)] bg-[color:var(--winey-surface)] px-4 py-3 text-center shadow-[var(--winey-shadow-sm)]">
          ...
          <p className="mt-1 text-[12px] text-[color:var(--winey-muted)]">
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {isMe ? <span className="text-[12px] text-[color:var(--winey-muted)]">(Admin)</span> : null}
                  {isHost ? (
                    <span
                      className={[
                        'rounded-[999px] border border-[color:var(--winey-border-strong)] px-2 py-[2px] text-[12px] font-semibold shadow-[var(--winey-shadow-sm)]',
                        submitted ? 'bg-[color:var(--winey-accent-green)] text-white' : 'bg-[color:var(--winey-warning)] text-white',
                      ].join(' ')}
                ...
                {!isMe ? (
                  <p className="mt-[2px] text-[12px] text-[color:var(--winey-muted)] truncate">
              ...
              {!isMe ? (
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className={[
                      'rounded-[var(--winey-radius-sm)] border border-[color:var(--winey-border-strong)] px-2 py-1 text-[12px] font-semibold text-white shadow-[var(--winey-shadow-sm)] transition-colors',
                      copiedUid === p.uid ? 'bg-[color:var(--winey-accent-green)] animate-pulse' : 'bg-[color:var(--winey-success)]',
                    ].join(' ')}
                    disabled={!isHost}
                  >
